...
def change_ins(index, to="nop"):
  global instr
  out = instr.copy()
  number = instr[index].split(" ")[1]
  out[index] = "{0} {1}".format(to, int(number))
  return out


def jmp_to_nop(instr):
  suspects = set()
  instr_use = instr.copy()
  while True:
    nh = never_halts(instr_use)
    if nh[0] and not nh[1] in suspects:
      suspects.add(nh[1])
      instr_use = change_ins(nh[1], "nop")
    elif not nh[0]:
      print(nh[1])
      print("We got one.")
      return
